...
def request(method: str, url: str, code: int, json=None, data=None, headers=None):
    res = requests.request(method, url, json=json, data=data, headers=headers)
...
def is_setup_done(args: dict) -> bool:
    ...
    res = request('GET', f'{base_url}/System/Info/Public', code=200)
...
def do_setup(args: dict):
    ...
    res = request(
        'POST', f'{base_url}/Startup/Configuration', code=204, json=data)

    res = request('GET', f'{base_url}/Startup/User', code=200, json=data)
    ...
    res = request('POST', f'{base_url}/Startup/User', code=204, json=data)
    ...
    res = request(
        'POST', f'{base_url}/Startup/RemoteAccess', code=204, json=data)

    res = request('POST', f'{base_url}/Startup/Complete', code=204)
...
def command_ldap(args: dict) -> dict:
    ...
    plugin = get_plugin_by_name(args, token, PLUGIN_LDAP_NAME)
    if plugin is None:
        return dict(failed=True, msg='LDAP plugin is not installed')

    if plugin['Status'] != 'Active':
        return dict(failed=True, msg='LDAP plugin is not active')
